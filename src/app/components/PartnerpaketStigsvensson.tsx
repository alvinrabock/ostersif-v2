...
export default async function PartnerpaketStigsvensson() {
  try {
    // Fetch the specific klubbstol package
    const { posts: allPartnerpaket } = await fetchPosts('partnerpaket', { limit: 500 });
    const klubbstolPackage = allPartnerpaket.find((paket: any) => paket.slug === 'klubbstol');
    ...
    // Transform klubbstol package to Foretagspaket type
    let foretagspaket: Foretagspaket | null = null;
    if (klubbstolPackage) {
      const paket = klubbstolPackage as any;
    ...
    return (
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-12">
        {/* Klubbstol Package Grid */}
        {foretagspaket && (
          <div>
            <h2 className="text-3xl font-bold text-white mb-8">Stig Svensson</h2>
    ...
    console.error('Error fetching Partnerpaket Stig Svensson:', error);
    return (
