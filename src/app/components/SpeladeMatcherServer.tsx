...
import SpeladeMatcherClient from "./SpeladeMatcherClient";

interface SpeladeMatcherServerProps {
    maxMatches?: number;
}

/**
 * Server Component that pre-fetches played matches
 * Data is static (finished matches don't change), so no client-side updates needed
 */
export default async function SpeladeMatcherServer({ maxMatches = 8 }: SpeladeMatcherServerProps) {
    try {
        ...
        // Pass pre-fetched data to client component (no live updates needed for finished matches)
        return <SpeladeMatcherClient matches={filteredMatches} />;
    } catch (error) {
